...
function renderCommitInfo(data, commits) {
  ...
  dl.append("dd").attr("id", "LOC").text(data.length);

  // Total commits
  dl.append("dt").text("Total commits");
  dl.append("dd").attr("id", "nCommits").text(commits.length);

  // Files
  dl.append("dt").text("Total files");
  const uniqueFiles = new Set(data.map((d) => d.file));
  dl.append("dd").attr("id", "nFiles").text(uniqueFiles.size);

  // Days since first commit
  dl.append("dt").text("Days since first commit");
  const firstDate = d3.min(commits, (d) => d.date);
  const lastDate = d3.max(commits, (d) => d.date);
  const daysSinceFirst = d3.timeDay.count(firstDate, lastDate);
  dl.append("dd").attr("id", "days").text(daysSinceFirst);
  ...
  dl.append("dd").attr("id", "fileLength").text(avgFileLength.toFixed(2));
}
  ...
  .html(
    (d, i) => `
      ...
      d3.rollups(
		...
		Then I looked over all I had made:  I killed it 😎.
`
  );
